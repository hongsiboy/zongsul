...
def objfun(x):
    ...
    history = model.fit([train_xd, train_xi], train_y, epochs=1000, batch_size=train_y.shape[0], 
...
for _ in range(2):
    x = transformer_block(x, head_size=transformer_head_size, num_heads=transformer_num_heads, 
                         ff_dim=transformer_ff_dim, dropout=transformer_dropout)
transformer_output = Dense(1)(x[:, -1, :])
...
mae_oma = mean_absolute_error(test_y, y_pred_OMA)
mse_oma = mean_squared_error(test_y, y_pred_OMA, squared=True)
rmse_oma = mean_squared_error(test_y, y_pred_OMA, squared=False)
...
mae_adam = mean_absolute_error(test_y, y_pred)
mse_adam = mean_squared_error(test_y, y_pred, squared=True)
rmse_adam = mean_squared_error(test_y, y_pred, squared=False)
...
# SHAP 분석
# OMA-최적화 모델의 예측 함수 정의
def model_predict(inputs):
    xd_flat = inputs[:, :15]  # Flatten된 xd (15 features)
    xi = inputs[:, 15:]       # xi (14 features)
    xd_reshaped = xd_flat.reshape(-1, 3, 5)  # 원래 shape으로 복원 (samples, 3, 5)
    concat_output = concat_model.predict([xd_reshaped, xi], verbose=0)
    predictions = expit(np.dot(concat_output, bestsol1))
    return predictions

# 데이터 준비
train_xd_flat = train_xd.reshape(train_xd.shape[0], -1)  # (samples, 15)
test_xd_flat = test_xd.reshape(test_xd.shape[0], -1)      # (samples, 15)
background_data = np.hstack([train_xd_flat, train_xi])[:100]  # (100, 29)
test_data = np.hstack([test_xd_flat, test_xi])  # (samples, 29)

# SHAP Explainer 생성
explainer = shap.KernelExplainer(model_predict, background_data)

# SHAP 값 계산 (테스트 데이터의 일부에 대해)
shap_values = explainer.shap_values(test_data[:10], nsamples=100)  # 10개 샘플만 사용
